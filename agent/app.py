...
def index():
    ...
    body = (f"<h1>Agents vs Wall Street — forecasting agent</h1>"
            f"<div class=sub><b>{methodology.METHODOLOGY_1['name']}</b> · "
            f"{manifest['n_filled']}/{manifest['n_total']} direct-output numbers produced. "
            f"Causal prequential evaluation is active on the ADI reference panel.</div>"
            f"<div class=card><a href='/graph'>▶ one pipeline · four companies · output layer (graph)</a>"
            f" &nbsp;·&nbsp; <a href='/graph?t=ADI'>ADI causal backtest →</a>"
...
def _graph_svg(fc, mf):
    ...
    parts.append(_rect(600, 488, 170, 40, "ensemble", "#1b222c", "#3a4658",
                       ["Capped ensemble", "prior-origin weights"]))
    ...
    nodes["backtest"] = {"t": "Causal prequential backtest · paired gate", "plug": "no",
                         "sub": "P4 · Backtesting (executes methodology)",
                         "body": "At each origin, predict from information available then. After a six-origin "
                                 "baseline-only warm-up, candidates must beat seasonal-naive by at least 5% "
                                 "on the same completed origins."}
    nodes["ensemble"] = {"t": "Capped causal ensemble", "plug": "no", "sub": "P4 · Backtesting",
                         "body": "Weights use only earlier completed origins: inverse paired relative MAE, "
                                 "model cap 0.60, and a 0.20 seasonal-naive floor."}

    # P5 Output
    parts.append(_rect(320, 584, 170, 40, "guardrail", "#16241b", "#2f6b43", ["Diagnostic MAE band", "point ± causal MAE"]))
    ...
    nodes["guardrail"] = {"t": "Diagnostic MAE band", "plug": "no", "sub": "P5 · Output",
    ...
    nodes["emit"] = {"t": "Workbook writer", "plug": "no", "sub": "P5 · Output",
                     "body": f"Writes submission/{fc['output_file']} — fills the Summary sheet and leaves its structure intact."}
    parts.append("</svg>")
